...
const Recipe = ({recipe,handelcooking}) => {
   ...
   console.log(recipe_image);
    return (
      <div className='flex flex-col py-5 px-5 border-1 border-[#28282833] shadow-2xs rounded-lg space-y-1  md:space-y-1'>

          <div className='flex-1 space-y-3 '>
          
<img src={recipe_image} alt=""  className=' h-40 md:h-48 lg:h-60 w-full object-cover  rounded-b-xl md:rounded-xl shadow-xl'/>
...
<div className='flex gap-x-5 capitalize mt-5'>
  ...
  <div onClick={()=>{handelcooking(recipe);}} 
  className="btn btn-success py-5 md:py-5 mt-5 
  md:px-5 font-semibold rounded-4xl md:text-lg
  text-[#150B2B] flex justify-center">Want to Cook</div>
      </div>
    );
};
